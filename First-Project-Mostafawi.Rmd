...
# Save the resulting dataset to an Excel file
write.xlsx(data, file = "M:/TeIAS/Semester 3/Development Economics/Projects/First Project/updated_dataset.xlsx")
...
clothing_long <- clothing %>%
  pivot_longer(cols = c("production", "sales", "directexportmilrial","real_exchangerate"), 
               names_to = "MainValues", 
               values_to = "MillionRial")
...
ggplot(clothing_long, aes(x = year, y = MillionRial, color = MainValues)) +
  ...
  scale_x_continuous(breaks = seq(min(clothing$year), max(clothing$year), by = 1)) +
  labs(
    title = "Real Exchange Rate and Main Values ",
    x = "Year",
    y = "MillionRial",
    color = "MainValues"
  ) +
